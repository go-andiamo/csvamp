...
field"`,
	...
	Input:  `§a∑""b,c`,
	Errors: []error{&csv.ParseError{Err: csv.ErrBareQuote}},
}, {
	Name:             "TrimQuote",
	Input:            ` §"a",§" b",§c`,
	Output:           [][]string{{"a", " b", "c"}},
	...
	Input:  `§"a ∑"word","b"`,
	Errors: []error{&csv.ParseError{Err: csv.ErrQuote}},
}, {
	Name:               "BadFieldCount",
	Input:              "§a,§b,§c\n¶∑§d,§e",
	Errors:             []error{nil, &csv.ParseError{Err: csv.ErrFieldCount}},
	...
	Input:              "§a,§b,§c\n¶∑§d,§e\n¶∑§f",
	Errors:             []error{nil, &csv.ParseError{Err: csv.ErrFieldCount}, &csv.ParseError{Err: csv.ErrFieldCount}},
	...
	Name:               "BadFieldCount1",
	Input:              `§∑a,§b,§c`,
	Errors:             []error{&csv.ParseError{Err: csv.ErrFieldCount}},
	Output:             [][]string{{"a", "b", "c"}},
	UseFieldsPerRecord: true,
	FieldsPerRecord:    2,
}, {
...
`,
	...
	Name:   "StartLine2",
	Input:  "§a,§b\n¶§\"d\n\n,e∑",
	Errors: []error{nil, &csv.ParseError{Err: csv.ErrQuote}},
	Output: [][]string{{"a", "b"}},
}, {
...
Aaa,Bbb,Ccc`
	...
	hdr, ok := r.Header()
	if !ok {
		t.Errorf("r.Header() returned false")
	}
	if strings.Join(hdr, ",") != "Foo,Bar,Baz" {
		t.Errorf("r.Header(): %s", strings.Join(hdr, ","))
	}
	raw := r.RawRecord()
	if string(raw) != "Aaa,Bbb,Ccc" {
		t.Errorf("raw record: %s", raw)
